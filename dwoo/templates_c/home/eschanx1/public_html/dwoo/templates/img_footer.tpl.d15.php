...
?>
		<?php 
$_fh7_data = (isset($this->scope["boardlist"]) ? $this->scope["boardlist"] : null);
if ($this->isArray($_fh7_data) === true)
{
	foreach ($_fh7_data as $this->scope['sect'])
	{
/* -- foreach start output */
?>
			[
			<?php 
$_fh6_data = (isset($this->scope["sect"]) ? $this->scope["sect"] : null);
$this->globals["foreach"]['brds'] = array
(
	"iteration"		=> 1,
	"last"		=> null,
	"total"		=> $this->isArray($_fh6_data) ? count($_fh6_data) : 0,
);
$_fh6_glob =& $this->globals["foreach"]['brds'];
if ($this->isArray($_fh6_data) === true)
{
	foreach ($_fh6_data as $this->scope['brd'])
	{
		$_fh6_glob["last"] = (string) ($_fh6_glob["iteration"] === $_fh6_glob["total"]);
/* -- foreach start output */
?>
...
<div class="footer" style="clear: both;">
	- <a href="http://kusabax.cultnet.net/" target="_top">kusaba x <?php echo KU_VERSION;?></a>
