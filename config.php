...
if (!$cache_loaded) {
	// Database
		...
		$cf['KU_DBDATABASE']      = 'eschanx1_db'; // Database... database. On SQLite this will be the path to your database file. Secure this file.
		$cf['KU_DBUSERNAME']      = 'eschanx1_admin'; // Database username. On SQLite this has no effect.
		$cf['KU_DBPASSWORD']      = 'admin0x'; // Database password. On SQLite this has no effect.
		$cf['KU_DBPREFIX']        = ''; // Database table prefix
		$cf['KU_DBUSEPERSISTENT'] = false; // Use persistent connection to database

	// Imageboard info
		$cf['KU_NAME']      = "Eschan X"; // The name of your site
		$cf['KU_SLOGAN']    = '<em>"Mejor que el nopor"</em>'; // Site slogan, set to nothing to disable its display
	...
	// Paths and URLs
		// Main installation directory
			...
			$cf['KU_WEBFOLDER'] = 'https://eschan.x10.mx/'; // The path from the domain of the board to the folder which kusaba is in, including the trailing slash.  Example: "http://www.yoursite.com/misc/kusaba/" would have a $cf['KU_WEBFOLDER'] of "/misc/kusaba/"
			$cf['KU_WEBPATH']   = 'https://eschan.x10.mx'; // The path to the index folder of kusaba, without trailing slash. Example: http://www.yoursite.com
			$cf['KU_DOMAIN']    = 'eschan.x10.mx'; // Used in cookies for the domain parameter.  Should be a period and then the top level domain, which will allow the cookies to be set for all subdomains.  For http://www.randomchan.org, the domain would be .randomchan.org; http://zachchan.freehost.com would be zach.freehost.com
